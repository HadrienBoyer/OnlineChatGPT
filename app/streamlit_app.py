...
def google_search(search: str, search_depth: int):
    try:
        ...
        st.warning("⛔︎ There was a problem with your internet.😵 \n\nIf you got HTTPError: 429, that means your IP is being rate limited by Google. If you are using a VPN try disabling it. \n Alternatively, you can try searching again another day…")
        raise
    ...
    with st.spinner(text="Searching the web…"):
        ...
        while links_explored < search_depth or links_attempted == len(links):
            links_attempted += 1
            if links == []:
                st.warning("⛔︎ No results found… ⛔︎")
                st.stop() 
...
st.set_page_config(page_title='⏣ Online ChatGPT ⏣')
st.title("⏣ Online ChatGPT ⏣")
st.markdown("By [HadrienB](https://github.com/HadrienBoyer)")
st.markdown('---')
